  ...
  <ul class="flex flex-col gap-12 md:flex-row">
    {#each data.videos as video}
      <a href={`${$page.url.pathname}/${video.label}`}>
        {#if video.thumbnail}
          {@const src = `${env.PUBLIC_POCKETBASE_URL}/api/files/${video.collectionId}/${video.id}/${video.thumbnail}?thumb=500x282`}

          <Image {src} label={video.label} />
        {:else}
          {@const src = `https://img.youtube.com/vi/${video.yt_url.replace(
